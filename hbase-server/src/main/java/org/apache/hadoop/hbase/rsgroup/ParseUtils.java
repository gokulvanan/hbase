...
import org.apache.hadoop.hbase.rsgroup.RSGroupAdminProto.ServerName;
import org.apache.hadoop.hbase.rsgroup.RSGroupAdminProto.RSGroupInfo.Builder;
...
public enum ParseUtils {
    ...
    public RSGroupInfo rsGroupFromProto(
            org.apache.hadoop.hbase.rsgroup.RSGroupAdminProto.RSGroupInfo rsGroupInfo) {
        RSGroupInfo output = new RSGroupInfo(rsGroupInfo.getName());
        Set<TableName> tables = new HashSet<>();
        for(org.apache.hadoop.hbase.rsgroup.RSGroupAdminProto.TableName tableProto : rsGroupInfo.getTablesList()){
        	tables.add(tableNameFromProto(tableProto));
        }
        output.addAllTables(tables);
        Set<HostAndPort> servers = new HashSet<>();
        for(org.apache.hadoop.hbase.rsgroup.RSGroupAdminProto.ServerName serverProto : rsGroupInfo.getServersList()){
    ...
    public org.apache.hadoop.hbase.rsgroup.RSGroupAdminProto.RSGroupInfo rsGroupToProto(
            RSGroupInfo rsGroupInfo) {
        Builder builder = org.apache.hadoop.hbase.rsgroup.RSGroupAdminProto.RSGroupInfo
                .newBuilder();
    ...
    public TableName tableNameFromProto(
            org.apache.hadoop.hbase.rsgroup.RSGroupAdminProto.TableName tableName) {
        return TableName.valueOf(tableName.getNamespace()
                .asReadOnlyByteBuffer(), tableName.getQualifier()
                .asReadOnlyByteBuffer());
    }

    public org.apache.hadoop.hbase.rsgroup.RSGroupAdminProto.TableName tableNameToProto(
            TableName table) {
        return org.apache.hadoop.hbase.rsgroup.RSGroupAdminProto.TableName
                .newBuilder()
